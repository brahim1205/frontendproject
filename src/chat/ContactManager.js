export class ContactManager {
  showAddContactModal(currentUser, apiService, onContactAdded) {
    ...
    document.getElementById('addContactForm').addEventListener('submit', async (e) => {
      ...
      saveBtn.innerHTML = '<i class="fas fa-spinner fa-spin mr-2"></i>Ajout...'
      
      try {
        // Vérifier si l'utilisateur existe
        const existingUser = await apiService.getUserByPhone(phoneNumber)
        if (!existingUser) {
          this.showNotification('Aucun utilisateur trouvé avec ce numéro de téléphone', 'error')
          return
        }
        
        // Vérifier si le contact n'existe pas déjà
        const existingContacts = await apiService.getContactsByUser(currentUser.id)
        const contactExists = existingContacts.some(contact => contact.phoneNumber === phoneNumber)
